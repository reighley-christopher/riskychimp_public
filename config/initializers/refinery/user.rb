::Refinery::ApplicationController.module_eval do
  ...
  def refinery_user_required?
    return true
    # Turn off checking of Refinery User
    #if just_installed? and controller_name != 'users' and !session[:redirected]
  ...
  def after_sign_in_path_for(resource_or_scope)
    ...
    #respond_to?(home_path, true) ? refinery.send(home_path) : refinery.root_path
  end
  ...
  def refinery_user?
    refinery_user_signed_in? && current_refinery_user.has_role?(:admin)
  end
